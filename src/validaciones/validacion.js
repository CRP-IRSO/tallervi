...
module.exports = {
    ...
    validacionDatosPUT: (data) => {
        const schema = yup.object().shape({
            ...
            nombres: yup.string(true).min(5).nullable(true).typeError('nombres must be a string').required('nombres is required'),
            ...
            telefono: yup.number(errores.error400).min(2).integer().nullable(true).typeError('telefono must be a number').required('telefono is required'), 
        });
        schema.validateSync(data);
    }
}
